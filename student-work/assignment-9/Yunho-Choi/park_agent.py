...
PARK_INFO = {
    ...
    "arches": {
        "state": "Utah",
        "best_season": "Spring (March-May) or Fall (September-October)",
        "top_activities": [
            "See Delicate Arch",
            "Hike to Landscape Arch",
            "Watch sunset at Balanced Rock"
        ],
        "things_to_pack": [
            "Sun hat",
            "Plenty of water",
        ...
        "nearby_airports": ["Moab, UT (CNY)", "Grand Junction, CO (GJT)"],
        "campgrounds": ["Devils Garden", "Sand Flats Recreation Area", "Canyonlands RV Resort"],
        "fun_fact": "Arches National Park has more than 2,000 natural stone arches."
    },
    "acadia": {
        "state": "Maine",
        "best_season": "Fall (September-October) or Summer",
        "top_activities": [
        ...
        "nearby_airports": ["Bangor, ME (BGR)", "Bar Harbor, ME (BHB)"],
        "campgrounds": ["Blackwoods", "Seawall", "Schoodic Woods"],
        "fun_fact": "Acadia is one of the first places in the U.S. to see the sunrise."
    }
}
...
Include a fun fact: {park['fun_fact']}"""
    ...
    print("\n[Agent] Step 4: Finding photography spots...\n")

    photo_prompt = f"""A traveler is visiting {park_name.title()} National Park.

Top activities: {', '.join(park['top_activities'])}
Best season: {park['best_season']}
Fun fact: {park['fun_fact']}

Suggest 3 photography spots or moments for this trip.
For each one, include a short reason why it is worth photographing.
Keep it practical and concise."""

    photo_spots = ask_llm(photo_prompt)
