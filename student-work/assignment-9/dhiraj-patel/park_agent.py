...
def save_trip_plan(park_name, itinerary, packing_list, logistics, summary):
    """Save the trip plan to a .txt file."""
    filename = f"{park_name.lower().replace(' ', '_')}_trip_plan.txt"
    
    with open(filename, 'w') as f:
        f.write(f"{'='*60}\n")
        f.write(f"  NATIONAL PARK TRIP PLAN: {park_name.title()}\n")
        f.write(f"{'='*60}\n\n")
...
Travel info: {logistics[:200]}"""
    ...
    save_trip_plan(park_name, itinerary, packing_list, logistics, summary)
