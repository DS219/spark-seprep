...
Travel info: {logistics[:200]}"""
    ...
    # STEP 5: Save the plan to a file (option D)
    with open(f"trip_plan_{park_name.title()}.md", "w") as f:
        f.write(summary + "\n\n" + packing_list + "\n\n" + logistics + "\n\n" + itinerary)
...
if get_park_info(park_choice):
    print("\nAsk follow-up questions about your trip!\n")
    while True:
        question = input("You: (type 'quit' or 'exit' to exit) ")

        if question.lower() in ["quit", "exit", "q"]:
            break
        
        # followup message about the trip (option C)
