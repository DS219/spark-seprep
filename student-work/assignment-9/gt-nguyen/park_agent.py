...
PARK_INFO = {
    "yellowstone": {
        ...
        "top_activities": ["Watch Old Faithful geyser", "Hike Grand Prismatic Spring", "Wildlife watching in Lamar Valley"],
    ...
    "grand canyon": {
        ...
        "top_activities": ["Hike Bright Angel Trail", "Watch sunrise at Mather Point", "Raft the Colorado River"],
    ...
    "zion": {
        "state": "Utah",
        "best_season": "Spring (March-May) or Fall (September-November)",
        "top_activities": ["Hike Angels Landing", "The Narrows river hike", "Scenic drive through the canyon"],
        "things_to_pack": ["Water shoes for The Narrows", "Trekking poles", "Sunscreen", "Refillable water bottle"],
    ...
    "great smoky mountains": {
        ...
        "top_activities": ["Drive Cades Cove Loop", "Hike to Clingmans Dome", "Visit Elkmont ghost town"],
...
def plan_trip(park_name):
    ...
    print("\n[Agent] Step 1: Creating a trip itinerary...\n")
    planning_prompt = f"""
You are a national park travel planner.

Create a simple 3-day trip itinerary for {park_name.title()} National Park.
...
Make the itinerary clear, practical, and friendly.
"""
...
Create a packing list for a trip to {park_name.title()} National Park.

Known items to pack:
{', '.join(park['things_to_pack'])}

Include a short explanation for why these items are useful.
"""
    packing_list = ask_llm(packing_prompt)
    print(packing_list)

    # Step 3: Travel tips
    print("\n[Agent] Step 3: Generating travel tips...\n")
    tips_prompt = f"""
You are a national park expert.

Give helpful travel tips for visiting {park_name.title()} National Park.
...
"""
    travel_tips = ask_llm(tips_prompt)
    print(travel_tips)

    # Step 4: Fun fact summary
    print("\n[Agent] Step 4: Sharing a fun fact...\n")
    print(park["fun_fact"])

    # Option D: Save the plan
    full_plan = f"""
Trip Plan for {park_name.title()} National Park
...
"""
    ...
    print(f"\n[Agent] Saved trip plan to {filename}")


print("Welcome to the National Park Trip Planner!")
print("Available parks: Yellowstone, Yosemite, Grand Canyon, Zion, Great Smoky Mountains")

park_choice = input("\nWhich park do you want to visit? ").lower()

if park_choice in PARK_INFO:
...
Question: {question}"""
    ...
    print("Sorry, that park is not in the database yet.")
