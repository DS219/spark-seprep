...
Travel info: {logistics[:200]}"""
    ...
    # STEP 5: Budget Estimates
    print("\n[Agent] Step 5: Figuring out budget estimates...\n")

    budget_prompt = f"""A traveler is visiting {park_name.title()} National Park in {park['state']}.

Nearby airports: {', '.join(park['nearby_airports'])}
Campground options: {', '.join(park['campgrounds'])}
itinerary: {itinerary[:300]}

Write a budget estimate based on flying into the nearest airport, renting a car, and camping for 2 nights, 
and participating in the activities from the itinerary."""
