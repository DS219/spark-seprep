...
def plan_trip(park_name):
    """
    Plan a trip to a national park by chaining multiple LLM steps.
    Returns the generated outputs so they can be reused later.
    """
...
Travel info: {logistics[:200]}"""
    ...
    # STEP 5: Wildlife / unique experiences
    print("\n[Agent] Step 5: Suggesting wildlife and experiences...\n")
    wildlife_prompt = f"""A traveler is visiting {park_name.title()} National Park.

Based on this trip summary:
{summary[:300]}

List 3-4 wildlife or unique experiences they might encounter.
Keep it short and interesting."""

    wildlife = ask_llm(wildlife_prompt)
    print("--- Wildlife & Experiences ---")
    print(wildlife)
...
trip_result = plan_trip(park_choice)
...
Here is their trip summary:
{trip_result['summary'][:300]}
