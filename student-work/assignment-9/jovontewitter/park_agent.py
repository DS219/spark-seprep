...
def ask_llm(prompt):
    ...
    response = requests.post(
        "http://localhost:11435/api/generate",  # 👈 important: your port
        json={
            "model": "smollm2:1.7b",
            "prompt": prompt,
            "stream": False
...
PARK_INFO = {
    ...
    "great smoky mountains": {
        ...
        "campgrounds": ["Cades Cove", "Elkmont", "Smokemont"], "fun_fact": "Great Smoky Mountains is the most visited national park in the US."
    },
    "acadia": {
        "state": "Maine",
        "best_season": "Fall for colors, Summer for hiking and coastal views",
        "top_activities": [
        ...
        "fun_fact": "Acadia protects much of Mount Desert Island and is famous for rocky Atlantic coastline views."
    },
    "rocky mountain": {
        "state": "Colorado",
        "best_season": "Summer for alpine trails, Fall for elk viewing",
        "top_activities": [
            "Drive Trail Ridge Road",
            "Hike to Dream Lake",
            "Watch elk in Estes Park"
        ],
        "things_to_pack": [
            "Warm layers",
            "Rain jacket",
        ...
        "fun_fact": "Rocky Mountain National Park reaches elevations above 14,000 feet."
    }
}
...
def plan_trip(park_name):
    """
    National Park Trip Planner Agent
    Steps:
      1. Create itinerary
      2. Build packing list
      3. Plan travel logistics
      4. Create summary
      5. Suggest wildlife to watch for
    """
...
Include this fun fact somewhere: {park['fun_fact']}"""
    ...
    summary_prompt = f"""You are a friendly travel agent. Combine these into a brief, exciting trip summary for {park_name.title()} National Park.

Use 3-4 sentences that would make someone excited to go.
...
{logistics[:200]}"""
    ...
    # STEP 5: Wildlife
    print("\n[Agent] Step 5: Suggesting wildlife you might see...\n")

    wildlife_prompt = f"""You are a national park guide.

The traveler is visiting {park_name.title()} National Park in {park['state']}.
Best season: {park['best_season']}
Planned activities: {', '.join(park['top_activities'])}

Write 3 short bullet points about wildlife they might see.
Then add one short wildlife safety reminder."""

    wildlife = ask_llm(wildlife_prompt)
    print("--- Wildlife You Might See ---")
    print(wildlife)
