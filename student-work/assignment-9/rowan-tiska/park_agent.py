...
PARK_INFO = {
    ...
    "great smoky mountains": {
        ...
        "acadia": {
        "state": "Maine",
        "best_season": "September and early October for smaller crowds; July and Augest peak season",
        "top_activities": ["Beehive Trail", "Precipice Trail",
                           "Jordan Pond"],
        "things_to_pack": ["Layered clothing", "rain jacket", "hiking boots", "sun protection"],
        "nearby_airports": ["Boston, MA (BOS)", "Hancock County-Bar Harbor Airport (BHB)"],
        "campgrounds": ["Blackwoods", "Schoodic"],
        "fun_fact": "One of the first places in the US to see the sunrise."
    },
        "grand tetons": {
        "state": "Wyoming",
        "best_season": "Mid-june to Augest; Late May to early June for best wildflowers and wildlife",
        "top_activities": ["Cascade Canyon", "Delta Lake",
                           "Phelps Lake"],
        "things_to_pack": ["rain jacket", "sun protection", "hat/gloves", "hiking boots"],
        "nearby_airports": ["Jackson Hole Airport (JAC)", "Idago Falls Regional Airport (IDA)"],
        "campgrounds": ["Colter Bay", "Gros Ventre", "Signal Mountain", "Lizard Creek", "Headwaters"],
        "fun_fact": "Young mountains, under 10 million years old formed by earthquakes."
    }
}
...
Include a fun fact: {park['fun_fact']}"""
    ...
    # changes ordering to make it flow better, added step 4 as step 5
    # STEP 4: Airbnb options near the park
    print("\n[Agent] Step 4: Finding areas for Airbnbs near the park...\n")

    airbnb_prompt = f"""A traveler is visiting {park_name.title()} National Park in {park['state']}.

Nearby airports: {', '.join(park['nearby_airports'])}
Top Activities: {', '.join(park['top_activities'])}

Write 2-3 options for where the best areas for Airbnbs are for this traveler, make sure its near
some of the ativities"""

    airbnb = ask_llm(airbnb_prompt)
    print("--- Airbnb Options ---")
    print(airbnb)
